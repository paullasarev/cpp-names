...
template <typename T>
std::string demangleNameOnly()
{
  std::string name(demangle<T>());
  size_t at = name.find_last_of(":");
...
struct Registrator
{
  ...
  {
    static const std::string name(demangle<Suite>() + "." + demangleNameOnly<Case>());
    return name;
  }
  static void Create()
  {
...
#define TESTCASE(Suite, Case)\
  struct Case: public yaffut::Test<Suite, Case>{public: Case(); }; \
  Case::Case()
